...
from app.orchestration.plugins import load_scenarios_text
...
def build_supervisor_system_prompt() -> str:
    ...
    base = (
        ...
        "(шаги, обязательные слоты, SQL в task, когда выбирать db или finish).\n"
        ...
        "Плейсхолдеры в SQL в точности как в сценарии не подменяй вручную — исполнитель подставит значения из slots "
        "по правилам выбранного сценария.\n"
        "Рекомендованный SQL из сценария помещай в task внутри блока ```sql ... ```, если сценарий это предписывает.\n"
